...
// Instantiate, check
TEST(UEStorePostgres, NewDestroy)
{
  ue_store* store = new ue_store_psql("localhost", "srsepc", "", "");

  delete store;
}

// Load Single Row
TEST(UEStorePostgres, SingleRow)
{
  ue_store* store = new ue_store_psql("localhost", "srsepc", "", "");
  ...
  const uint8_t opc[] = {0x6b, 0x34, 0x44, 0xc5, 0x83, 0x5a, 0x5d, 0xf7, 0xd3, 0xad, 0xd5, 0x7e, 0x85, 0xb7, 0x70, 0xe3};
